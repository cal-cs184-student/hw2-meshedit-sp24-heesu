...
namespace CGL
{
    ...
    void MeshResampler::upsample(HalfedgeMesh& mesh)
    {
        ...
        for (EdgeIter e = mesh.edgesBegin(); e != mesh.edgesEnd(); ++e) {
            ...
            auto computeEdgeNewPosition = [](const EdgeIter& e) {
                Vector3D pos1 = e->halfedge()->vertex()->position;
                Vector3D pos2 = e->halfedge()->twin()->vertex()->position;
                Vector3D pos3 = e->halfedge()->next()->next()->vertex()->position;
                Vector3D pos4 = e->halfedge()->twin()->next()->next()->vertex()->position;
                return (3.0 / 8.0) * (pos1 + pos2) + (1.0 / 8.0) * (pos3 + pos4);
                };

            e->newPosition = computeEdgeNewPosition(e);
        }

        // Vertex precomputation: Set isNew flags and compute new positions
        for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); ++v) {
            v->isNew = false;

            Vector3D sumNeighborPos;
            float n = 0.0; // Neighbor count
            for (auto h = v->halfedge(); ; h = h->twin()->next()) {
                sumNeighborPos += h->twin()->vertex()->position;
                n += 1.0;
                if (h->twin()->next() == v->halfedge()) break;
            }
            float u = (n == 3.0) ? 3.0 / 16.0 : 3.0 / (8.0 * n);
            v->newPosition = (1.0 - n * u) * v->position + u * sumNeighborPos;
        }

        // Split edges and position new vertices
        vector<EdgeIter> edgesToSplit;
        for (EdgeIter e = mesh.edgesBegin(); e != mesh.edgesEnd(); ++e) {
            if (!e->isNew) edgesToSplit.push_back(e);
        }
        ...
        // Flip edges based on new and old vertices
        for (EdgeIter e = mesh.edgesBegin(); e != mesh.edgesEnd(); ++e) {
            if (e->isNew) {
        ...
        // Update vertex positions to new positions
        for (VertexIter v = mesh.verticesBegin(); v != mesh.verticesEnd(); ++v) {
